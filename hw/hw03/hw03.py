...
def count_dollars(total):
    ...
    def helper(remaining, current_bill):
        ...
        if remaining < 0 or current_bill is None:
            return 0
        use_current = helper(remaining - current_bill, current_bill)
        skip_current = helper(remaining, next_smaller_dollar(current_bill))
...
def count_dollars_upward(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def f(sum, cur, target):
        if sum == target:
            return 1
        if sum > target or cur is None:
            return 0
        use_cur = f(sum + cur, cur, target)
        skip_cur = f(sum, next_larger_dollar(cur), target)
        return use_cur + skip_cur
    return f(0, 1, total)
